...
public static class AuthenticatedUserResourceFromEntityAssembler
{
    ...
    ///     Thrown if <paramref name="user" /> is null or <paramref name="token" /> is null.
    /// </exception>
    /// <exception cref="ArgumentException">
    ///     Thrown if <paramref name="token" /> is empty.
    /// </exception>
    public static AuthenticatedUserResource ToResourceFromEntity(User user, string token)
    {
        ...
        {
            throw new ArgumentNullException("Token cannot be null or empty when creating authenticated user resource.",
                nameof(token));
        }
